...
> 与「对齐谁」的关系：对齐谁 = API 风格跟随谁；复用优先级 = 实现参考谁。二者独立（API 对齐 antdv 但实现可复用 naive 的某段算法）。
> ⚠️ **代码注释无品牌红线（2026-09-22 修订）**：参考来源**不写进代码注释**——❌ 不写「对齐 antd 口径」「参考 naive 实现」「仿 antd」等以品牌名作依据的句子（原表述「在代码注释注明来源」**已废**）；改为中性表述（本项目统一约定 → 直接写明约定本身）或直接删。来源与决策理由落 `devlog` / 工作上下文「决策记录」。唯一例外：`@ant-design/*` 基础包（图标/色板）不属参考库组件。详见 `references/refine-spec.md` §1，脚本 `C5` 拦截。
> ⚠️ **「复用项目已有组件」的判断依据是真实依赖**：若组件内部并未 import 其它组件（如 Comment 只用原生 `<img>`），**不要为了「复用」而强行引入**（会在 `components/utils/style-deps.ts` 的 `componentDependencies` 新增样式依赖 + 组件耦合）；参考库不套子组件时，本项目也不套。
