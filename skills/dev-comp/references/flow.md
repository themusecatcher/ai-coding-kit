...
2. **用例结构（核心要求）**：
   - **完整复制 antdv 官网全部展示用例**：官网组件页（如 `https://www.antdv.com/components/{组件名}-cn/`）的每个展示用例都要在演示页中有对应分区，**分区顺序按已确认排序锚点排列**（锚点 A 默认＝与官网展示用例顺序一致；锚点 B＝合理性优先，见 `refine-spec.md` §4.1），只保留**用例标题 + 必要描述**（仿照项目其他组件演示页的写法，不复制大段官网文案）
...
5. **清除对照 + 交付前精修（红线 · ⭐ 2026-09-22 扩展 · 2026-09-23 补范围口径）**：本步一次性完成「清品牌 → 精修三处产物 → 三方对账」，**权威源 `references/refine-spec.md`**（判定标准 / 改写范式 / 对照矩阵），执行顺序不可颠倒（先清避免白改、后对保证三方一致）。
   ⚠️ **精修范围 = 四个来源**（`refine-spec.md` §0.1）：①组件源码 `components/{组件名}/**` ②演示页 `src/views/{组件名}/` ③docs（组件文档 + changelog/features/index）④**git 已提交批次**。**先取范围清单**：`git diff --name-only <base>`（base = `--base` / 工作上下文 `base_ref` / `merge-base 主干`）**+ `git ls-files --others --exclude-standard`**，一次覆盖「已提交的每一批 + 未提交改动」（脚本 `G0` 已自动按三载体分类输出该清单）；**❌ 只看 `git diff`（工作区）在分批提交场景会整段漏检已提交批次；❌ 不得以 grep 抽检代替逐载体通读**；清单里每个文件都要被逐项核对，结论写进「交付前精修记录」。范围与本轮改了哪些文件**无关**。
   ...
   d) **演示用例精修**（§4）：**排序锚点先确认**（§4.1：A 官网保序＝默认，官网用例保序 / 新增插回原序 / 特有用例归末尾；B 合理性优先＝主题聚簇 + 由浅入深 / 特有用例就近插入）；布局——单例整块 / 2+ 同构示例并排 / **交互类单独全宽**，**同类型用例布局必须一致**；分区结构统一（`<h2 class="mt30 mb10">` → 可选 `<p class="mb10">` → 示例容器）
...
| G | ⭐ **G0 精修范围清单**（四来源：组件源码 / 演示页 / docs / git 已提交批次；取 `git diff --name-only <base>` + 未跟踪文件） | | 脚本 G0 输出的三载体文件清单 + **逐项核对结论** |
| G | G1 组件源码注释精修（三层结构 / 无复述与过期注释 / 无品牌来源标注） | | 人工（refine-spec §2）——**须留逐文件核对证据（逐层确认），grep 抽检不算完成** |
| G | G2 Props 排序（六段式分组 + 段内语义相邻 + 与 docs 表逐项同序） | | 脚本 G2 |
| G | G3 演示用例排序与布局（锚点先确认：官网保序 / 合理性优先 + 同类型布局一致） | | 人工（refine-spec §4）——**须留逐区布局分类结论，抽检不算完成** |
...
| 项 | 状态 | 证据/说明 |
|----|:--:|------|
| **精修范围确认**（四来源清单：组件源码 / 演示页 / docs / git 已提交批次） | | 脚本 G0 的三载体文件清单 + **逐项核对结论**（⚠️ 「本轮未改动」≠「无需核对」） |
