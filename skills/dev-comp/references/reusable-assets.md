...
> **核心原则**：组件库中已有的**功能/样式/布局/逻辑**优先复用，**禁止重新开发**新的组件、样式或逻辑。

## 常见可复用能力检索清单（先搜后造）

> 开发组件遇到以下能力时，**先 `search_content`/浏览 `components/` 确认项目是否已有实现**，有则复用（可直接引用组件或抽取逻辑），无则按复用优先级参考 antdv/naive 实现。下表为常见检索项示例，路径以实际检索结果为准：

| 需求能力 | 常见既有实现（检索关键词） | 复用方式 |
|:--|:--|:--|
| 滚动条 | Scrollbar / scrollbar | 直接引用既有滚动条组件或复用其样式 |
| 暂无数据 | Empty / empty / no-data | 直接引用既有空态组件 |
| 清除按钮 | clearable / clear（输入框/下拉框组件的清除实现） | 复用既有组件的清除按钮样式与逻辑 |
| 下拉列表 | Dropdown / Select 的弹出列表实现 | 复用既有下拉结构，不重写 |
| 加载中 | Spin / Loading | 直接引用既有加载组件 |

> ⚠️ 检索确认不存在时，按 `reference-sources.md` §实现方式复用优先级 2→3→4 级处理，并在工作上下文「可复用项目资产」区记录检索结论（存在则记录位置，不存在则记录「已检索，无既有实现」）。
