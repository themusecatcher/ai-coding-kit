...
- ⚠️ **条目无品牌红线（2026-09-22 新增）**：条目**不得以参考库品牌名作参照**——❌ 不写「属性命名对齐 [Ant Design Vue](…antdv.com…)」「全量对齐 [Naive UI]」这类表述；直接写本项目自身的变更事实（如「`search` 更名为 `showSearch`、`filter` 更名为 `filterOption`（旧名移除）」）。changelog 是用户可见文档，也是品牌残留**高发区**（实测 `select` 分支 2 条命中），权威源与处置规则见 `refine-spec.md` §1.4 A 类（**整段删除品牌指代**），脚本 `C5` 拦截。
