...
> 🔴 **可复制容器（强制）**：每个版本的 commit message 主体**必须用「text 围栏代码块」包裹**（三反引号 + `text`），让 IDE 渲染出复制按钮。
> 标题层（`📝` 汇总 + `📋`/`📄` 子标题）用**加粗**强调，**不带任何分割线**；**仅 commit message 主体入代码块**。
...
最终输出形态（标题层用加粗强调，**无分割线**；commit message 主体在 ```text 代码块内，IDE 渲染复制按钮）：
