...
### v4 → v5 核心变更（2026-07-22）

> **交互体验与自检体系完善**：v5 在 v4 执行层硬化基础上，新增交互式菜单命令 `dev:ask`、帮助一致性检查 `dev:help --check`，以及 dev-flow 自身盲区清单。

> **核心变更点**：
>
> - **`dev:ask` / `dev:guide` 交互式菜单**：全功能分类菜单入口，支持状态感知（活跃流程/已完成上下文/git 仓库检测），覆盖所有 dev-flow 功能，不进入步骤流程
> - **`dev:help --check` 帮助一致性检查**：命令行脚本对比 `help.md` 与权威源（SKILL.md / mode-matrix.md / step-router.md）的一致性
> - **源文件索引更新**：新增 `branch-recommendation.md`、`menu.md`、`help-consistency.sh` 等文件

---

## 1. 入口触发与模式路由

> **v5 更新**：新增 `dev:ask`/`dev:guide` 交互式菜单、`dev:help --check` 帮助一致性检查。
...
| `dev:ask` | `dev:guide` | 交互式功能菜单（全功能分类入口，状态感知，不进入步骤流程） |
| `dev:help` | `dev:h` | 显示帮助信息；`--check` 运行一致性检查 |
...
B -->|"dev:ask / dev:guide"| MENU["📋 交互式功能菜单<br/>（状态感知，不进入步骤流程）"]
B -->|"dev:help / dev:h / --check"| HELP["❓ 帮助/一致性检查"]
...
classDef menu fill:#0f172a,stroke:#818cf8,color:#c7d2fe
class C,FLOW unified
class MICRO micro
class DRIFT drift
class SYNC,SUB,ITER_CMD cmd
class MENU,HELP menu
class R,RD,RM,STD,FULL iter
```

> **v5 触发原则**：dev-flow 仅由显式命令触发或活跃流程相关恢复触发。AI 不得基于关键词主观判断自动触发。
> **新增命令**：`dev:ask`/`dev:guide` 交互式分类菜单（加载 `references/menu.md`，含状态感知 4 项检测）、`dev:help --check` 帮助一致性检查（对比 help.md 与权威源）。
...
> **v5 无变更**：产出物真实性原则（证伪式标注 + 极简模式准入）+ 交互式选项一致性 `interactive_progression_shown` 字段均为 v4 已引入。
...
> **v5 无变更**：micro-fix 升级（≤3 文件 + 对称修改 + 5 项 safe defaults）+ 文档平台 × 模式处理策略矩阵均为 v4 已引入。
...
> **v5 无变更**：产出物真实性原则影响步骤 3 产出的形态（简单任务允许 1 句话方案，但须通过机校验准入清单）。
...
> **v5 无变更**：环节 K（dev-logs 完整性自检）已在 v3 引入。v4 新增 5.5b 新鲜度物理兜底（`freshness-lint.sh` 在步骤 5.5 执行而非步骤 7）。
...
> **v5 无变更**：分支推荐仍唯一定稿在步骤 4 §4.1。产出物以需求确认为核心，极简任务允许精简输出。
...
## 12. 交互模式（精简交互 + 口语意图消歧 + 交互式菜单）

> **v5 新增**：`dev:ask`/`dev:guide` 交互式菜单——提供全功能分类入口，含状态感知（活跃流程/已完成上下文/git 仓库检测），覆盖所有 dev-flow 功能，不进入步骤流程。
...
> **v5 新增**：`help-consistency.sh`（帮助一致性检查）。
...
> - **help-consistency.sh**（帮助一致性）：对比 help.md 与权威源的一致性
...
| `references/menu.md` | `dev:ask`/`dev:guide` 交互式功能菜单（状态感知+双重展示，不进入步骤流程） |
| `references/help.md` | 帮助命令单一权威源（`dev:help`/`dev:h` 加载） |
...
| `references/branch-recommendation.md` | 分支命名推荐输出模板与命名约束（步骤4 §4.1 细节下沉） |
...
| `references/cross-project/handoff.md` | 跨项目衔接 prompt（B 项目入口规范） |
| `config/gates.yaml` | 门控规则单一权威源（物理检查点+lint+hooks+状态机+工具门禁+freshness） |
| `config/hooks.json` | Hook 注册表 |
| `scripts/` | 程序化执行层（含 health-check.sh/freshness-lint.sh/help-consistency.sh + 65+个脚本） |
