...
> 与 知识库平台/remote_kb 远程知识库互补——本 skill 是 AI 的「肌肉记忆」（本地、可写、AI 沉淀），知识库平台 是项目的「全景地图」（远程、只读、源码语义检索）。
